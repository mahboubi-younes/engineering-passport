...
const unknown = (detail = 'Evidence unavailable') => ({ status: 'unknown', detail });
...
function technologiesFromProject(list, fileList, manifests, hasPython) {
...
async function githubEvidence() {
  ...
  return {
    ...
    workflows: latestNonPassport
      ? (latestNonPassport.conclusion === 'success'
        ? verified('Latest non-Passport workflow succeeded on ' + latestNonPassport.updated_at)
        : detected('Latest non-Passport workflow: ' + (latestNonPassport.conclusion || latestNonPassport.status)))
      : null,
    latestRun: latestNonPassport ? {
      name: latestNonPassport.name,
      status: latestNonPassport.status,
      conclusion: latestNonPassport.conclusion,
      updatedAt: latestNonPassport.updated_at,
      url: latestNonPassport.html_url
    } : null
  };
}
...
const statusText = (value) =>
  value.status === 'verified' ? 'VERIFIED' :
...
export async function generate({ root, outputDir = root }) {
  ...
  const workflowContents = await Promise.all(
    projectWorkflowFiles.map(async (relativePath) => ({
      path: relativePath,
      content: await read(path.join(root, relativePath))
    }))
  );
  ...
  const pagesUrl = github.pages?.status === 'verified'
    ? github.pages.detail
    : null;
  const demoUrl = demoLink?.url || pagesUrl;
  const demoHttpStatus = await checkUrl(demoUrl);

  const technologies = [];
  technologiesFromProject(technologies, fileList, manifests, hasPython);

  const buildManifest = manifests.find(({ data }) => typeof data.scripts?.build === 'string');
  const buildWorkflow = workflowContents.some(({ content }) =>
    /npm\s+(?:run\s+)?build|yarn\s+build|pnpm\s+build/i.test(content)
  );
  const testWorkflow = workflowContents.some(({ content }) =>
    /npm\s+(?:run\s+)?test|yarn\s+test|pnpm\s+test|\bpytest\b|\bvitest\b|\bplaywright\b/i.test(content)
  );

  const tests = testFiles.length === 0
    ? unknown('No test files or test configuration detected in the repository')
  ...
  const hasStaticEntry = fileList.some((f) => /^index\.html$/i.test(f));

  const buildEvidence = buildManifest
    ? (buildWorkflow && github.latestRun?.conclusion === 'success'
      ? verified('Build script detected and latest relevant workflow succeeded')
      : detected('Build script declared in ' + buildManifest.path + ': ' + buildManifest.data.scripts.build))
    : (fileList.some((f) =>
        /(^|\/)(vite\.config\.|webpack\.config\.|rollup\.config\.|Makefile|Dockerfile)$/i.test(f)
      )
      ? detected('Build configuration detected')
      : hasStaticEntry
        ? detected('Static browser project; no build script is required or declared')
        : unknown());

  const ciEvidence = workflowFiles.length === 0
    ? unknown('No GitHub Actions workflow detected')
    : github.latestRun?.conclusion === 'success'
      ? verified('Latest non-Passport workflow succeeded on ' + github.latestRun.updatedAt)
      : github.latestRun
        ? detected('Workflow detected; latest non-Passport run: ' + (github.latestRun.conclusion || github.latestRun.status))
        : detected(workflowFiles.length + ' GitHub Actions workflow file(s) detected');

  const liveDemoEvidence = demoUrl
    ? (demoHttpStatus
      ? verified('Live demo reachable; HTTP ' + demoHttpStatus)
      : detected('Live demo URL detected but could not be verified during this run'))
    : unknown('Live demo URL not detected');
